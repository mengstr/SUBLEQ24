...
bool loadfrom(istream &in)
{
	while(1)
	{
		...
		if( s[0]=='#' )
		{
		  ...
		  if( s.size()==siin.size()+1 && s.substr(1)==siin ) mem[ip++] = iin;
		  else
		  {
...
int main(int ac, char *av[])
{
	...
	for( int i=1; i<ac; i++ )
	{
		...
		if( s.size()>8 && s.substr(1,7)=="inname=" ) siin = s.substr(8);
		else
	...
	if( trace ) cout<<"using outname="<<sout<<" outaddr="<<iout<<" inname="<<siin<<" inaddr="<<iin<<'\n';
